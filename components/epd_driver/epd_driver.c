...
void epd_copy_to_framebuffer(EpdRect image_area, const uint8_t *image_data, uint8_t *framebuffer) {
    ...
    for (uint32_t i = 0; i < image_area.width * image_area.height; i++) {
        ...
        uint8_t val =
            (value_index % 2) ? (image_data[value_index / 2] & 0xF0) >> 4 : image_data[value_index / 2] & 0x0F;
...
static void draw_rotated_transparent_image(EpdRect        image_area,
...
void epd_draw_rotated_transparent_image(EpdRect        image_area,
                                        const uint8_t *image_buffer,
                                        uint8_t *      framebuffer,
                                        uint8_t        transparent_color) {
