...
EpdiyHighlevelState epd_hl_init(const EpdWaveform* waveform) {
    ...
    */
    ...
    state.back_fb = heap_caps_malloc(fb_size, MALLOC_CAP_8BIT);
    assert(state.back_fb != NULL);
    state.front_fb = heap_caps_malloc(fb_size, MALLOC_CAP_8BIT);
    assert(state.front_fb != NULL);
    state.difference_fb = heap_caps_malloc(2 * fb_size, MALLOC_CAP_8BIT);
...
enum EpdDrawError epd_hl_update_area(EpdiyHighlevelState* state, enum EpdDrawMode mode, int temperature, EpdRect area) {
    ...
    EpdRect diff_area = epd_difference_image_cropped(state->front_fb,
                                                     state->back_fb,
                                                     area,
                                                     state->difference_fb,
                                                     state->dirty_lines,
    ...
    if (previously_white) {
        err = epd_draw_base(epd_full_screen(),
                            state->front_fb,
                            diff_area,
        ...
        err = epd_draw_base(epd_full_screen(),
                            state->front_fb,
                            diff_area,
                            MODE_PACKING_2PPB | PREVIOUSLY_BLACK | mode,
                            temperature,
                            state->dirty_lines,
                            state->waveform);
    } else {
